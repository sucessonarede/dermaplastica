...
### October 13, 2025
- **Added search and date filters to saved quotes page**: The /apresentacao page now includes comprehensive filtering capabilities. Users can search quotes by patient name using the search input, filter by date range using start and end date pickers, and clear all filters with a single click. The UI shows a results counter when filters are active and displays an appropriate empty state when no quotes match the criteria. This makes it easy to locate specific presentations even with a large volume of saved quotes.
- **Added discount pricing system**: Implemented comprehensive discount pricing for procedures. Added optional "Preço c/ Desconto (Unid/mL)" field to procedure forms. When a procedure has a discounted price, the procedure card displays the original price with strikethrough (small, muted) and the discounted price in green (large, bold). QuoteBuilder automatically uses discounted prices in subtotal calculations (priority: customPrice > discountedPrice > mlPrice/basePrice). Presentation slide 6 (Plano Personalizado) shows both prices with clear visual distinction - original price × quantity strikethrough (small) and discounted price highlighted in green (text-xl). This creates a compelling value proposition when presenting to patients.
- **Hidden prices during protocol building**: Transformed QuoteBuilder into a consultative experience where prices are hidden during protocol creation. Prices are now only revealed in the formal presentation to the patient. Changes include: removed all price displays from procedure cards and quantity controls, removed price breakdowns and totals from the summary section, kept procedure names, quantities, and notes visible. The button was renamed from "Salvar Orçamento" to "Salvar e Apresentar" and now automatically opens the presentation after saving. This allows clinics to focus on patient needs during consultation, with prices presented professionally in the dedicated presentation module.
- **Fixed cache management on authentication**: Added proper cache clearing on login and registration flows using `queryClient.clear()`. Previously only logout was clearing cache, which could cause data leakage between users or show stale data. Now all authentication flows (login, register, logout) consistently clear the React Query cache to ensure fresh data and prevent security issues.
- **Added procedure-level observations/notes**: Users can now add custom text observations to each procedure in the quote summary (e.g., "região malar direita", "aplicação na testa"). Notes display with a MessageSquare icon and can be edited inline. Notes are fully persisted to the database and restored when loading saved quotes. Notes also appear in the saved quote presentation (slide 6 - Plano Personalizado) below each procedure name in italic, muted text. This allows clinics to specify detailed application areas or special instructions for each procedure that are visible both during quote creation and in client presentations.
